...
const SalesListPDF: React.FC<Props> = ({ billList }) => (
    <Document>
        <Page size="A4" style={styles.page}>
            <Text style={styles.title}>Sales Bill List</Text>
            <View style={styles.table}>
                <View style={styles.row}>
                    ...
                    <Text style={[styles.col, styles.colPayment, styles.header]}>Payment</Text>
                    <Text style={[styles.col, styles.colComments, styles.header]}>Comments</Text>
                </View>
                {billList.map((c, idx) => (
                    <View style={styles.row} key={c.id}>
                        ...
                        <Text style={[styles.col, styles.colBill]}>
                            {c.billNumber}
                            <Text style={styles.finYear}>{`(${formatFinYear(c.financialYear)})`}</Text>
                        </Text>
                        <Text style={[styles.col, styles.colDate]}>{formatDate(c.billDate) || "-"}</Text>
                        <Text style={[styles.col, styles.colNet]}>{formatCurrency(c.totalBeforeTax)}</Text>
                        <Text style={[styles.col, styles.colGST]}>{formatCurrency(c.totalGST)}</Text>
                        <Text style={[styles.col, styles.colTotal]}>{formatCurrency(c.totalAmount)}</Text>
                        <Text style={[styles.col, styles.colPayment]}>{c.paymentStatus || "-"}</Text>
                        <Text style={[styles.col, styles.colComments]}>{c.comments || "-"}</Text>
                    </View>
                ))}
            </View>
        </Page>
